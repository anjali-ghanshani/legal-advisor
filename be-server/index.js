...
app.get("/appointments/:appointment_id" ,async(req,res) =>{
    try {
        ...
        const appointments = await pool.query("SELECT * FROM appointment WHERE appointment_id = $1",[appointment_id])
...
app.put("/appointments/:appointment_id", async(req,res) => {
    try{
        ...
        const updateAppointment = await pool.query("UPDATE appointment SET app_stime = $1 WHERE appointment_id = $2",[app_stime,appointment_id])
...
app.delete("/appointments/:appointment_id", async(req,res) => {
    try{
        const {appointment_id} = req.params;
        const deleteAppointment = await pool.query("DELETE FROM appointment WHERE appointment_id = $1 ",[appointment_id])
